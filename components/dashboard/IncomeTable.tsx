// Income Table component for displaying and editing positive cash flows (income)
...
type IncomeEntry = ExistingIncomeEntry | NewIncomeEntry

export function IncomeTable({ incomes, planId, className }: IncomeTableProps) {
  ...
  // Get the effective value for a field (edit if exists, otherwise from income/newEntry)
  const getValue = (
    entry: IncomeEntry,
  ...
  const handleNewEntryAmountChange = (tempId: string, value: string) => {
    const numericValue = parseFloat(value.replace(/[^0-9.-]/g, '')) || 0
    handleNewEntryChange(tempId, 'amount', numericValue)
  }
  ...
  const handleSave = async () => {
    ...
    Object.entries(edits).forEach(([id, edit]) => {
      const income = incomes.find((inc) => inc.id === parseInt(id, 10))
      if (income && !validateEntry(edit, income)) {
    ...
    Object.entries(edits).forEach(([id, edit]) => {
      ...
      if (!isNaN(numericId)) {
        const income = incomes.find((inc) => inc.id === numericId)
        if (income) {
          // Merge edit with original income data
          const updateData: Omit<CashFlow, 'id' | 'plan_id'> = {
            ...
            end_date: edit.end_date ?? income.end_date ?? undefined,
          }
          updates.push(
    ...
    try {
      ...
      console.error('Failed to save income entries:', error)
      alert('Failed to save income entries. Please try again.')
    }
  }
  ...
  // Combine existing incomes and new entries for display
  const allEntries: IncomeEntry[] = [
    ...incomes
      .filter((inc): inc is CashFlow & { id: number } => inc.id !== undefined)
      .map((inc) => ({ type: 'existing' as const, id: inc.id, data: inc })),
            ...
            >
              ...
              <td className="px-4 py-3">
                <div className="relative">
                  ...
                  <input
                    type="text"
                    value={formatCurrency(getValue(entry, 'amount') || 0)}
                    onChange={(e) =>
                      isNew
